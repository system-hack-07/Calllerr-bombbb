...
# === YOUR FULL API LIST (add all remaining ones here) ===
ULTIMATE_APIS = [
...
<head>
    ...
    <title>Samarth Sms+Call Bomber</title>
    ...
    <div class="max-w-6xl mx-auto p-8">
        <div class="flex justify-between items-center mb-12">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 bg-red-600 rounded-2xl flex items-center justify-center text-4xl">💥</div>
                <div>
                    <h1 class="text-5xl font-bold">Samarth Bomber</h1>
                    <p class="text-emerald-400 text-xl">SMS + CALL + WHATSAPP INFINITE</p>
                </div>
            </div>
            <div id="status" class="text-emerald-400 font-mono text-lg">READY</div>
        </div>

        <div class="grid grid-cols-12 gap-6">
            <div class="col-span-12 lg:col-span-5 glass rounded-3xl p-10">
                <h2 class="text-3xl mb-8">Target Number</h2>
                <input id="phone" maxlength="10" class="w-full bg-zinc-900 border border-zinc-700 rounded-2xl px-6 py-5 text-3xl font-mono tracking-widest outline-none" placeholder="9876543210">
                <div class="flex gap-4 mt-8">
                    <button onclick="startAttack()" id="startBtn" class="flex-1 bg-red-600 hover:bg-red-700 py-6 rounded-2xl text-xl font-bold">🚀 START INFINITE BOOM</button>
                    <button onclick="stopAttack()" id="stopBtn" class="hidden flex-1 bg-slate-700 hover:bg-red-600 py-6 rounded-2xl text-xl font-bold">🛑 STOP ATTACK</button>
                </div>
            </div>

            <div class="col-span-12 lg:col-span-7 glass rounded-3xl p-8">
                <h3 class="text-xl mb-6">LIVE STATS</h3>
                <div class="grid grid-cols-3 gap-6 text-center">
                    <div><div id="calls" class="text-5xl font-bold text-orange-400">0</div><div class="text-slate-400">CALLS</div></div>
                    <div><div id="sms" class="text-5xl font-bold text-blue-400">0</div><div class="text-slate-400">SMS</div></div>
                    <div><div id="wa" class="text-5xl font-bold text-green-400">0</div><div class="text-slate-400">WHATSAPP</div></div>
                </div>
                <div class="mt-10">
                    <div class="flex justify-between mb-2 text-sm"><span>CYCLES</span><span id="cycles">0</span></div>
                    <div class="h-2 bg-zinc-800 rounded-full"><div id="progress" class="h-2 bg-red-500 w-0 transition-all"></div></div>
                </div>
            </div>

            <div class="col-span-12 glass rounded-3xl p-8">
                <h3 class="mb-4 text-xl">ATTACK LOGS</h3>
                <div id="logs" class="font-mono text-sm h-80 overflow-auto bg-black/40 p-4 rounded-2xl"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function pollStatus() {
            ...
            fetch("/status").then(r => r.json()).then(d => {
                ...
                document.getElementById("progress").style.width = Math.min(d.cycles * 5 % 100 + 30, 100) + "%";
